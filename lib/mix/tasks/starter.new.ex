defmodule Mix.Tasks.Starter.New do
  @shortdoc "Generates your project's starter"
  ...
  # The starter written into new projects: every built-in step, grouped and
  ...
  def igniter(igniter) do
    ...
    |> Igniter.create_new_file(
      "lib/mix/tasks/#{app_name}.starter.ex",
      starter_source(app_module, body, origin)
    )
    |> Igniter.add_notice("""
    Your starter lives at lib/mix/tasks/#{app_name}.starter.ex — every step is
  ...
  defp starter_source(app_module, body, origin) do
    origin_note =
      if origin do
        " from `#{origin}` — edits here do not affect the shared starter"
      else
        ""
      end
    ...
    if Code.ensure_loaded?(Starter) do
      defmodule Mix.Tasks.#{app_module}.Starter do
        @shortdoc "Runs this project's starter"

        @moduledoc \"\"\"
        This project's starter, generated by `mix starter.new`#{origin_note}.
  ...
  defp resolve_starter!(from) do
    module = Module.concat([from])

    if Code.ensure_loaded?(module) and function_exported?(module, :__starter__?, 0) do
      module
    else
      Mix.raise("""
      #{from} is not a starter.

      Pass a module that uses Starter and is available in this
  ...
  defp describe({:starter, module}) do
    {"{:starter, #{inspect(module)}}", "Includes the #{inspect(module)} starter"}
  end

  defp describe({:starter, module, [if: flag]}) do
    {"{:starter, #{inspect(module)}, if: :#{flag}}",
     "Includes the #{inspect(module)} starter #{flag_note(flag)}"}
  end
  ...
  defp shortdoc(kind, name) do
    case Starter.Steps.resolve(kind, name) do
      {:ok, module} -> Mix.Task.shortdoc(module) || to_string(name)
      :error -> raise "starter references unknown step: {#{kind}, #{name}}"
    end
  end
